
import numpy as np
import matplotlib.pyplot as plt
from scipy.signal import argrelextrema
from scipy.ndimage import gaussian_filter1d
from numba import njit


@njit
def precompute_powers(a, b, n_terms):
    a_powers = np.empty(n_terms)
    b_powers = np.empty(n_terms)
    a_val = 1.0
    b_val = 1.0
    for n in range(n_terms):
        a_powers[n] = a_val
        b_powers[n] = b_val
        a_val *= a
        b_val *= b
    return a_powers, b_powers


@njit
def precompute_weierstrass(t, a_powers, b_powers):
    n_terms = len(a_powers)
    W = np.empty(len(t))
    for i in range(len(t)):
        total = 0.0
        for n in range(n_terms):
            total += a_powers[n] * np.cos(np.pi * b_powers[n] * t[i])
        W[i] = total
    return W


@njit
def solve_oscillator(t, omega0, hk, hd, Wk, Wd, dt):
    n_steps = len(t)
    x = np.zeros(n_steps)
    v = np.zeros(n_steps)
    x[0] = 1.0
    v[0] = 0.0
    for i in range(1, n_steps):
        omega_sq = omega0**2 * (1 + hk * Wk[i - 1])
        gamma = hd * Wd[i - 1]
        a = -omega_sq * x[i - 1] - gamma * v[i - 1]
        v[i] = v[i - 1] + a * dt
        x[i] = x[i - 1] + v[i] * dt
    return x, v


@njit
def compute_lyapunov(t, omega0, hk, hd, Wk, Wd, dt, delta0=1e-8):
    n_steps = len(t)
    x = 1.0
    x_perturbed = x + delta0
    v = 0.0
    v_perturbed = v
    sum_log_divergence = 0.0

    for i in range(1, n_steps):
        omega_sq = omega0**2 * (1 + hk * Wk[i - 1])
        gamma = hd * Wd[i - 1]

        a = -omega_sq * x - gamma * v
        a_perturbed = -omega_sq * x_perturbed - gamma * v_perturbed

        v_new = v + a * dt
        x_new = x + v_new * dt

        v_perturbed_new = v_perturbed + a_perturbed * dt
        x_perturbed_new = x_perturbed + v_perturbed_new * dt

        dx = x_perturbed_new - x_new
        dv = v_perturbed_new - v_new
        delta = np.sqrt(dx * dx + dv * dv)

        if delta == 0.0:
            continue

        sum_log_divergence += np.log(delta / delta0)
        scale = delta0 / delta
        x_perturbed_new = x_new + dx * scale
        v_perturbed_new = v_new + dv * scale

        x, v = x_new, v_new
        x_perturbed, v_perturbed = x_perturbed_new, v_perturbed_new

    return sum_log_divergence / (t[-1] - t[0])


def get_maxima(x, t, discard_ratio=0.5):
    idx_peaks = argrelextrema(x, np.greater)[0]
    if len(idx_peaks) == 0:
        return np.array([])
    discard_time = t[0] + discard_ratio * (t[-1] - t[0])
    filtered_peaks = [x[i] for i in idx_peaks if t[i] > discard_time]
    return np.array(filtered_peaks)


def plot_bifurcation_and_lyapunov(h_vals, t, omega0, a, b, n_terms, dt):
    a_powers, b_powers = precompute_powers(a, b, n_terms)
    Wk = precompute_weierstrass(t, a_powers, b_powers)
    Wd = precompute_weierstrass(t + 1.234, a_powers, b_powers)  # phase-shifted for damping

    lyap_vals = []

    fig, axs = plt.subplots(1, 2, figsize=(14, 5))

    for hk in h_vals:
        hd = hk  # use same modulation strength for damping
        x, v = solve_oscillator(t, omega0, hk, hd, Wk, Wd, dt)
        peaks = get_maxima(x, t)
        lyap = compute_lyapunov(t, omega0, hk, hd, Wk, Wd, dt)
        lyap_vals.append(lyap)

        color = 'red' if lyap > 0 else 'blue'
        axs[0].plot([hk] * len(peaks), peaks, '.', color=color, alpha=0.2, markersize=0.7)

    lyap_vals = np.array(lyap_vals)
    lyap_smooth = gaussian_filter1d(lyap_vals, sigma=2)

    axs[0].set_xlabel('h (modulation strength)')
    axs[0].set_ylabel('x(t) maxima')
    axs[0].set_title('Bifurcation Diagram (Color = Chaos/Stability)')
    axs[0].grid(True)

    axs[1].plot(h_vals, lyap_smooth, 'k-', linewidth=1)
    axs[1].axhline(0, color='gray', linestyle='--')
    axs[1].fill_between(h_vals, lyap_smooth, where=(lyap_smooth >= 0), color='red', alpha=0.3)
    axs[1].fill_between(h_vals, lyap_smooth, where=(lyap_smooth < 0), color='blue', alpha=0.3)

    axs[1].set_xlabel('h (modulation strength)')
    axs[1].set_ylabel('Lyapunov Exponent')
    axs[1].set_title('Smoothed Lyapunov Exponent vs h')
    axs[1].grid(True)

    plt.tight_layout()
    plt.show()


# Parameters
omega0 = 1.0
a = 0.7
b = 9
n_terms = 30
dt = 0.001
T_max = 500
t = np.arange(0, T_max, dt)
h_vals = np.linspace(0.0, 1.0, 400)

plot_bifurcation_and_lyapunov(h_vals, t, omega0, a, b, n_terms, dt)
