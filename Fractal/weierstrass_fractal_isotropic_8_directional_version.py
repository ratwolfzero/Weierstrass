...
# --- Numba-accelerated 2D Weierstrass function (isotropic 8-directional) ---
@njit(parallel=True)
def compute_weierstrass_2d(X, Y, a_powers, b_freqs):
    """Compute the 2D Weierstrass function (isotropic 8-directional)."""
    W = np.zeros_like(X)
    angles = np.array([0, np.pi/4, np.pi/2, 3*np.pi/4, np.pi, 
                       5*np.pi/4, 3*np.pi/2, 7*np.pi/4])
    for n in prange(len(a_powers)):  # Parallel loop
        term = np.zeros_like(X)
        for theta in angles:
            direction = X * np.cos(theta) + Y * np.sin(theta)
            term += np.cos(b_freqs[n] * direction)
        W += a_powers[n] * (term / 8.0)  # Average over 8 directions
    return W
...
def update_plot(val):
    ...
    # Compute Weierstrass function using the 8-directional isotropic version
