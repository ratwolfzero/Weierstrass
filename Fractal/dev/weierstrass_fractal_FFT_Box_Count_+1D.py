...
# Right bottom: FFT of 1D function
ax2 = plt.subplot(gs[1, 1])
line_fft, = ax2.semilogy([], [], 'r-')
ax2.set_title('FFT of 1D Weierstrass')
...
# --- Update function ---
def update_plot(val):
    global current_Z_norm, current_1d_data, current_dimension, current_plot, dimension_calculated, last_a, last_b
    a = slider_a.val
    b = slider_b.val
    ab = a * b
    ...
    # Update FFT plot
    line_fft.set_data(pos_freqs, pos_fft)
    ax2.set_xlim(0, np.max(pos_freqs))
    ax2.set_ylim(np.min(pos_fft[pos_fft > 0]), np.max(pos_fft)*1.1)
